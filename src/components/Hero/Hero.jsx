import React from "react";
import { getImageUrl } from "../../utils";
import styles from "./Hero.module.css";

export const Hero =() => {
    return <section className={styles.container}>
        <div className={styles.content}>
            <h1 className={styles.title}>Hi, I'm Huiting</h1>
            <p className={styles.description}>I’m a motivated bilingual programmer skilled in object oriented programming, web development, and test automation. Reach out if you’d like to learn more!</p>
            <a href="mailto:hhtbetty@outlook.com" className={styles.contactBtn}>Contact Me</a>
        </div>
        <img src={getImageUrl("hero/heroImage.png")} alt="HeroImage" className={styles.heroImg}/>
        <div className={styles.topBlur} />
        <div className={styles.bottomBlur} />
        </section>
};